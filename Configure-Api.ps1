...
# 可选参数：提供 -GamePath/-Provider/-ApiKey/-Model 时跳过对应交互（便于自动化）
param(
    [string]$GamePath = "",
    [string]$Provider = "",
    [string]$ApiKey = "",
...
if ([string]::IsNullOrWhiteSpace($Provider)) {
    ...
    for ($i = 0; $i -lt $providers.Count; $i++) {
        Write-Host "  $($i + 1)) $($providers[$i].Name)（$($providers[$i].BaseUrl)）"
    }
...
if ([string]::IsNullOrWhiteSpace($Model)) {
    ...
    $modelInput = Read-Host "请输入模型名（直接回车使用默认：$defaultModel）"
